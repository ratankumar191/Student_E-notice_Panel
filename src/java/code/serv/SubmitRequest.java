    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try 
        {
            ...
            else
            {
                ...
                String oldrole=rc.checkRole(v);
                Boolean status=rc.requestChange(v,oldrole,newrole);
                 if(status)
                 {
                    session.setAttribute("userId",v);
                    request.setAttribute("message","Request Sent!");
                RequestDispatcher rd=request.getRequestDispatcher("ChangeMyRole.jsp");
                rd.forward(request,response);
                 }
                 else
                 {
                    session.setAttribute("userId",v);
                    request.setAttribute("message","Failed To Send Request!");
                RequestDispatcher rd=request.getRequestDispatcher("ChangeMyRole.jsp");
                rd.forward(request,response);
                 }
            }
